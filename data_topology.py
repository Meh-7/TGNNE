...
def _accept_simplex(
    ...
    """
    decide whether to accept a simplex under per-entity caps.

    note:
        using per-entity caps can break the topological structure,
        because some simplices may be rejected while their cofaces or faces
        are still accepted. this violates face inclusion and can lead to
        a non-simplicial complex.

        this could be fixed later by enforcing face inclusion (adding all
        faces of any accepted simplex)
        ...
        gudhi might also have options to enforce this during simplex tree construction. or fixing this later.
    """
...
def extract_simplicial_structure(
    ...
    uses igraph.Graph.cliques(min=3, max=4) on the undirected 1-skeleton.
    ...
    if ig is None:
        raise ImportError(
            "igraph is not installed but is required for extract_simplicial_structure. "
            "Install with `pip install igraph` or add a networkx fallback."
        )

    # build igraph graph from edge_index
    # edge_index contains undirected edges with u < v
    edges = edge_index.t().tolist()  # list of (u, v) pairs
    g = ig.Graph(n=num_entities, edges=edges, directed=False)

    logger.info(
        "running igraph clique enumeration on graph with %d nodes and %d edges",
        g.vcount(),
        g.ecount(),
    )

    triangles_set: Set[Tuple[int, int, int]] = set()
    tetras_set: Set[Tuple[int, int, int, int]] = set()

    tri_counts: Dict[int, int] = {}
    tet_counts: Dict[int, int] = {}

    # single pass: enumerate all 3- and 4-cliques
    for clique in g.cliques(min=3, max=4):
        k = len(clique)
        if k == 3:
            tri = tuple(sorted(int(v) for v in clique))
            if _accept_simplex(tri, tri_counts, max_triangles_per_entity):
                triangles_set.add(tri)
        elif k == 4:
            tet = tuple(sorted(int(v) for v in clique))
            if _accept_simplex(tet, tet_counts, max_tetras_per_entity):
                tetras_set.add(tet)

    # sort for deterministic output
    triangles_list = sorted(triangles_set)
    tetras_list = sorted(tetras_set)

    logger.info(
        "enumerated simplices (igraph): %d triangles and %d tetrahedra (pre-simplex-tree)",
        len(triangles_list),
        len(tetras_list),
    )
...
def build_topology_from_triples(
    ...
    steps:
        ...
        2) extract triangles and tetrahedra via clique enumeration (igraph)
    ...
    else:  # this is in case of implementing a proper optional check for the simplex tree
        # fallback: construct tensors directly from clique enumeration output
        logger.warning("using fallback tensors from clique enumeration output (no simplex tree)")
